...
class TestBBcAppClient(object):
    ...
    def test_00_setup(self):
        ...
        for i in range(core_num):
            ...
            make_client_sync(index=i, core_port_increment=0, callback=msg_processor[i])
        time.sleep(1)
    ...
    def test_07_search_asset0(self):
        ...
        dat = clients[0]['app'].search_transaction_with_condition(asset_group_id=asset_group_id, asset_id=asid)
        assert dat[KeyType.status] == ESUCCESS
        print(dat)
    ...
    def test_08_search_asset_not_found(self):
        ...
        print(" search for asset:%s"%binascii.b2a_hex(asid))
        print("* should be NG *")
        dat = clients[0]['app'].search_transaction_with_condition(asset_group_id=asset_group_id, asset_id=bytes(asid))
        assert dat[KeyType.status] < ESUCCESS
    ...
    def test_15_search_transaction(self):
        ...
        dat = clients[0]['app'].search_transaction(transactions[0].transaction_id)
        assert dat[KeyType.status] == ESUCCESS
    ...
    def test_16_search_transaction(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        print("* should be NG *")
        dat = clients[0]['app'].search_transaction(b'4898g9fh')
        assert dat[KeyType.status] < ESUCCESS
