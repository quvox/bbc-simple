...
class TestBBcAppClient(object):
    ...
    def test_03_insert(self):
        ...
        for i in range(20):
            ...
            clients[1]['app'].insert_transaction(transactions2[i])
            dat = clients[1]['app'].callback.synchronize()
            assert KeyType.transaction_id in dat
    ...
    def test_05_search_transaction_direction_forward(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        clients[2]['app'].traverse_transactions(transactions1[1].transaction_id, direction=0, hop_count=3)
        dat = clients[2]['app'].callback.synchronize()
        assert dat[KeyType.status] == 0
        assert KeyType.transaction_tree in dat
    ...
    def test_06_search_transaction_direction_backward(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        clients[3]['app'].traverse_transactions(transactions2[4].transaction_id, direction=1, hop_count=3)
        dat = clients[3]['app'].callback.synchronize()
        assert dat[KeyType.status] == 0
        assert KeyType.transaction_tree in dat
        assert len(dat[KeyType.transaction_tree]) == 3
        asset_bodies = list()
    ...
    def test_07_search_transaction_direction_forward(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        clients[4]['app'].traverse_transactions(transactions2[4].transaction_id, direction=0, hop_count=10)
