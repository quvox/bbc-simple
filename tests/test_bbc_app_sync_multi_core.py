...
from bbc_simple.core import bbc_app_sync
from testutils import prepare, get_core_client, start_core_thread, make_client_sync, domain_setup_utility
...
class MessageProcessor(bbc_app_sync.Callback):
...
class TestBBcAppClient(object):
    ...
    def test_00_setup(self):
        ...
        for i in range(client_num):
            ...
            make_client_sync(index=i, core_port_increment=i, callback=msg_processor[i])
        time.sleep(1)
    ...
    def test_13_insert_first_transaction(self):
        ...
        dat = clients[0]['app'].insert_transaction(transactions[0])
    ...
    def test_13_gather_signature(self):
        ...
        dat = clients[1]['app'].callback.synchronize()  # ここは本当は非同期メッセージでやってくるはず
        ...
        dat = clients[1]['app'].insert_transaction(transactions[1])
        assert KeyType.transaction_id in dat
        assert dat[KeyType.transaction_id] == transactions[1].transaction_id

    def test_17_search_asset(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        dat = clients[2]['app'].search_transaction_with_condition(asset_group_id=asset_group_id,
    ...
    def test_18_search_asset(self):
        ...
        dat = clients[3]['app'].search_transaction_with_condition(asset_group_id=asset_group_id, asset_id=bytes(asid))
        assert KeyType.transactions not in dat
        assert KeyType.all_asset_files not in dat

    def test_19_search_asset(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        dat = clients[0]['app'].search_transaction_with_condition(asset_group_id=asset_group_id,
    ...
    def test_20_search_transaction(self):
        ...
        dat = clients[4]['app'].search_transaction(transactions[0].transaction_id)
        assert dat[KeyType.status] == 0
        assert KeyType.transaction_id in dat
        assert dat[KeyType.transaction_id] == transactions[0].transaction_id
    ...
    def test_21_search_transaction(self):
        print("\n-----", sys._getframe().f_code.co_name, "-----")
        print("* should be NG *")
        dat = clients[4]['app'].search_transaction(b'4898g9fh')  # NG is expected
        assert dat[KeyType.status] < 0
    ...
    def test_97_get_stat(self):
        ...
        import pprint
        for cl in clients:
            dat = cl['app'].get_stats()
            pprint.pprint(dat[KeyType.stats])
