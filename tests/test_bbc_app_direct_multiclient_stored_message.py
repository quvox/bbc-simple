...
class TestBBcAppClient(object):
    ...
    def test_00_setup(self):
        ...
        for i in range(core_num):
            domain_setup_utility(i, domain_id, host="bbc1.securityhub.jp")  # system administrator
        for i in range(client_num):
            msg_processor[i] = MessageProcessor(index=i)
            make_client_direct(i, host="bbc1.securityhub.jp", port=9000, callback=msg_processor[i])
        time.sleep(1)
